    ...
    try {
        ...
        if (uncategorized.length > 0) {
            result.push({
                id: "_uncategorized",
                name: "Uncategorized",
                items: await Promise.all(uncategorized.map(async (i) => ({
                    id: i.cloudId,
                    name: i.name,
                    basePrice: i.priceCents,
                    description: null,
...
app.get("/items/:id", async (req, reply) => {
    ...
    if (cloud && !cloud.deletedAt) {
        ...
        const priceMap = new Map(sizePrices.map((p) => [`${p.baseType}|${p.sizeOptionCloudId}`, p.priceCents]));
        ...
        const itemOptionGroupsRaw = links.map((link) => {
            ...
            const defaultOptCloudId = g.defaultOptionCloudId ?? cloud.defaultSizeOptionCloudId;
