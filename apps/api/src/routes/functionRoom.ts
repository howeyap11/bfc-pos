...
export const functionRoomRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/function-room/start", { preHandler: app.requireStaff }, async (req, reply) => {
    ...
    if (!frTable) return reply.code(400).send({ error: "FR_TABLE_NOT_FOUND" });
  ...
  app.get("/function-room/active", { preHandler: app.requireStaff }, async (req, reply) => {
    ...
    if (!tab) return reply.send({ active: false });
