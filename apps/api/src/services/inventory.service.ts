...
export type InventoryMovementPostInput = {
  ...
  /** STORE = IngredientStock; WAREHOUSE = IngredientWarehouseStock. Default STORE. */
  locationCode?: StockLocationCodeValue | null;
...
export class InventoryService {
  ...
  private effectiveStockLocation(locationCode?: string | null): StockLocationCodeValue {
   ...
   */
  private pulloutStockLegFromRow(row: {
    locationCode: string | null;
    type: InventoryMovementType;
  }): StockLocationCodeValue | null {
    const raw = row.locationCode;
    if (raw != null && String(raw).trim() !== "") {
      return this.effectiveStockLocation(raw);
    }
    if (row.type === "TRANSFER_OUT") return StockLocationCode.WAREHOUSE;
    if (row.type === "TRANSFER_IN") return StockLocationCode.STORE;
    return null;
  }
  ...
  private async applyStockDeltaTx(
    ...
    params: { storeId: string; ingredientId: string; delta: Decimal; locationCode: StockLocationCodeValue }
  ): Promise<void> {
   ...
   */
  async postMovement(params: InventoryMovementPostInput) {
    const {
      storeId,
      ingredientId,
      type,
    ...
    if (unitId !== ingredient.unitId) {
      throw new Error(
        `Unit mismatch: Movement unit ${unitId} does not match ingredient unit ${ingredient.unitId}. Unit conversion not yet supported.`
      );
    }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const stock =
        loc === StockLocationCode.WAREHOUSE
  ...
  async getOnHand(params: { storeId: string; ingredientId: string }): Promise<Decimal> {
   ...
   */
  async recalcStockFromLedger(params: { storeId: string; ingredientId: string }) {
    const { storeId, ingredientId } = params;
    const ingredient = await this.prisma.ingredient.findUnique({
      where: { id: ingredientId },
  ...
  async getOperationalStockSnapshot(params: {
    ...
    Array<{
      ingredientId: string;
      cloudIngredientCloudId: string | null;
      name: string;
      unitId: string;
      storeOnHandQty: string;
      warehouseOnHandQty: string;
    }>
  > {
  ...
  async postMovementsBatch(movements: InventoryMovementPostInput[]) {
    const results = await this.prisma.$transaction(async (tx) => {
      const batchResults: Array<{ movement: unknown; stock: unknown }> = [];
      for (const params of movements) {
        ...
        if (unitId !== ingredient.unitId) {
          throw new Error(
            `Unit mismatch for ingredient ${ingredientId}: Movement unit ${unitId} does not match ingredient unit ${ingredient.unitId}`
          );
        }
        const loc = this.effectiveStockLocation(locationCode);
        const movement = await tx.inventoryMovement.create({
  ...
  async consumeForSale(params: {
    ...
    if (recipes.length === 0 && sizeRecipes.length === 0) return [];

    const agg = new Map<string, { unitId: string; totalQty: Decimal }>();
    for (const line of lineItems) {
      const qty = Math.max(0, Math.trunc(line.qty || 1));
      if (qty === 0) continue;

      const hasSize = !!(line.baseType && line.sizeCode);
      const applicableSizeRecipes =
        hasSize
          ? sizeRecipes.filter(
              (rec) =>
                rec.menuItemId === line.itemId &&
      ...
      const rowsToUse =
        applicableSizeRecipes.length > 0
    ...
    const movements: InventoryMovementPostInput[] = [];
   ...
   */
  private async ensurePosCloudLedgerAnchorIfMissing(params: {
    storeId: string;
    refType: "POS_CLOUD_SALE" | "POS_CLOUD_REFUND" | "POS_CLOUD_VOID";
    refId: string;
    notes: string;
    createdByStaffId?: string;
  }): Promise<void> {
  ...
  private async posLedgerMarkerIfUnmapped(params: {
    storeId: string;
    refType: "POS_CLOUD_SALE" | "POS_CLOUD_REFUND" | "POS_CLOUD_VOID";
    refId: string;
    movementsCreated: number;
    hadNonZeroQty: boolean;
    createdByStaffId?: string;
    unmappedCloudIngredientIds?: string[];
  }): Promise<void> {
    const { storeId, refType, refId, movementsCreated, hadNonZeroQty, createdByStaffId, unmappedCloudIngredientIds } =
      params;
    if (movementsCreated > 0 || !hadNonZeroQty) return;
    const anyIng = await this.prisma.ingredient.findFirst({
      where: { storeId },
      orderBy: { id: "asc" },
      select: { id: true, unitId: true },
    ...
    if (!anyIng) return;
    const idList =
      unmappedCloudIngredientIds && unmappedCloudIngredientIds.length > 0
  ...
  async applyPosCloudSaleDeductions(params: {
    ...
    createdByStaffId?: string;
    /** Structured audit: unmapped cloud ingredient ids on sale finalize */
    ...
    const movements: InventoryMovementPostInput[] = [];
    const unmappedCloudIngredientIds: string[] = [];
  ...
  async applyWasteReportDeduction(params: {
    ...
    inventoryItemCloudId: string | null;
    quantityStr: string;
    createdByStaffId?: string;
  }): Promise<void> {
    ...
    if (exists) return;
    if (!inventoryItemCloudId?.trim()) return;
    let qty: Decimal;
    try {
    ...
    if (!qty.isFinite() || qty.lte(0)) return;
  ...
  async staffStoreAdd(params: {
    ...
    if (!ing) throw new Error(`Ingredient not found for cloud id ${ingredientCloudId}`);
    await this.postMovement({
      storeId,
      ingredientId: ing.id,
      type: "ADJUSTMENT",
  ...
  async staffWarehousePulloutToStore(params: {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const legs = new Set<StockLocationCodeValue>();
      for (const e of existing) {
        const leg = this.pulloutStockLegFromRow(e);
        if (leg) legs.add(leg);
      }
      if (legs.has(StockLocationCode.WAREHOUSE) && legs.has(StockLocationCode.STORE)) return;
  ...
  async getStockLevels(params: { storeId: string; ingredientIds?: string[] }) {
    const { storeId, ingredientIds } = params;
    const where: { storeId: string; ingredientId?: { in: string[] } } = { storeId };
