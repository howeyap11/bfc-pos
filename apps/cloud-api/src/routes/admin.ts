...
async function ensureUniqueSlug(prisma: { category: { findUnique: (args: { where: { slug: string } }) => Promise<{ id: string } | null> } }, baseSlug: string, excludeId?: string): Promise<string> {
...
export async function adminRoutes(app: FastifyInstance) {
  ...
  app.post("/settings/admin-accounts", async (req: FastifyRequest, reply: FastifyReply) => {
    if (!requireCloudAdmin(req, reply)) return;
  ...
  app.delete("/settings/admin-accounts/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    if (!requireCloudAdmin(req, reply)) return;
    const { id } = req.params;
  ...
  app.get("/settings/receipt-details", async (req: FastifyRequest, reply: FastifyReply) => {
    if (!requireCloudAdmin(req, reply)) return;
  ...
  app.put("/settings/receipt-details", async (req: FastifyRequest, reply: FastifyReply) => {
    if (!requireCloudAdmin(req, reply)) return;
    ...
    if (locked && !turningOffBir) {
      const regChanged =
        (body.taxType !== undefined && body.taxType !== existing.taxType) ||
    ...
    if (body.receiptMessage !== undefined) data.receiptMessage = body.receiptMessage;
    if (body.birEnabled !== undefined) data.birEnabled = body.birEnabled;
    if (!locked || turningOffBir) {
      if (body.taxType !== undefined) data.taxType = body.taxType;
      if (body.permitNo !== undefined) data.permitNo = body.permitNo;
      if (body.issueDate !== undefined) data.issueDate = body.issueDate;
      if (body.nonVatTin !== undefined) data.nonVatTin = body.nonVatTin;
      if (body.vatTin !== undefined) data.vatTin = body.vatTin;
      if (body.birMin !== undefined) data.birMin = body.birMin;
      if (body.birSerialNo !== undefined) data.birSerialNo = body.birSerialNo;
    }
  ...
  app.put("/settings/sales-inventory", async (req: FastifyRequest, reply: FastifyReply) => {
    if (!requireCloudAdmin(req, reply)) return;
    ...
    if (body.dailySalesEmailTimeLocal !== undefined) data.dailySalesEmailTimeLocal = body.dailySalesEmailTimeLocal;
  ...
  app.post("/staff", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  app.patch("/staff/:id", async (req: FastifyRequest<{ Params: { id: string }; Body: unknown }>, reply: FastifyReply) => {
  ...
  app.get("/menu-settings/sizes", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const variants: { id: string; mode: string; sizeId: string; sizeLabel: string; imageUrl: string | null; sortOrder: number }[] = [];

    for (const row of rows) {
      if (!row.size.isActive) continue;
  ...
  app.post("/menu-settings/sizes", async (req: FastifyRequest, reply: FastifyReply) => {
    const parsed = z.object({
  ...
  app.patch("/menu-settings/sizes/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    const parsed = z.object({
  ...
  app.post("/menu-settings/sizes/availability/:id/image", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
  ...
  app.delete("/menu-settings/sizes/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    const group = await app.prisma.menuSettingGroup.findUnique({
      where: { key: SIZES_GROUP_KEY },
      include: { menuSizes: true },
  ...
  app.put("/menu-settings/sizes/availability", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    for (const mode of ["ICED", "HOT", "CONCENTRATED"] as const) {
    ...
    await app.prisma.$transaction(async (tx) => {
      ...
      for (const mode of modes) {
        ...
        if (sizeIdsForMode.length === 0) continue;
  ...
  app.post("/menu-settings/shots", async (req: FastifyRequest, reply: FastifyReply) => {
    const parsed = z
      .object({
  ...
  app.patch("/menu-settings/shots/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    const parsed = z
      .object({
        name: z.string().optional(),
  ...
  const handleTxError = (err: unknown, reply: FastifyReply) => {
  ...
  app.get("/menu-settings/transaction-types", async (req: FastifyRequest, reply: FastifyReply) => {
    const tx = txDelegate();
    if (!tx) {
      reply.code(503);
  ...
  app.post("/menu-settings/transaction-types", async (req: FastifyRequest, reply: FastifyReply) => {
    const parsed = z
      .object({
        label: z.string().min(1),
  ...
  app.patch("/menu-settings/transaction-types/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const tx = txDelegate();
    if (!tx) {
      reply.code(503);
      return { error: "PRISMA_CLIENT_OUTDATED", message: "Run 'pnpm exec prisma generate' in apps/cloud-api and restart the API." };
  ...
  app.post("/cleanup-legacy-sizes", async (req: FastifyRequest, reply: FastifyReply) => {
    if (!requireCloudAdmin(req, reply)) return;
  ...
  app.patch("/items/:id", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const updateData: {
      ...
      defaultSubstituteOptionId?: string | null;
      version: number;
  ...
  app.put("/items/:id/drink-sizes", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    ...
    for (const mode of modes) {
      ...
      if (enabledOptionIds.length > 0) anyEnabled = true;
    }
  ...
  app.post("/ingredient-categories", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  app.delete("/ingredient-categories/:id", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as { id: string };
  ...
  app.post("/ingredients", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get("/ingredients/:id", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as { id: string };
  ...
  app.patch("/ingredients/:id", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as { id: string };
  ...
  app.delete("/ingredients/:id", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as { id: string };
    const version = await bumpCatalogVersion(app.prisma);
  ...
  app.post("/ingredients/reorder", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    await app.prisma.$transaction(
      parsed.data.order.map(({ id, sortOrder }) =>
        app.prisma.ingredient.update({ where: { id }, data: { sortOrder } })
      )
    );
    return { ok: true };
  });
  ...
  app.post("/ingredients/:id/image", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
  ...
  app.post("/inventory/locations", async (req: FastifyRequest, reply: FastifyReply) => {
    const parsed = z.object({
      code: z.string().min(1),
      name: z.string().min(1),
  ...
  app.post("/inventory/transfers", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  app.patch("/inventory/transfers/:id/approve", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as { id: string };
  ...
  app.post("/inventory/reconciliations", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  app.patch("/inventory/reconciliations/:id/approve", async (req: FastifyRequest, reply: FastifyReply) => {
    const { id } = req.params as { id: string };
  ...
  app.post("/inventory/dev-manual-set", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  app.put("/items/:id/recipe", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (item.hasSizes && parsed.data.sizeLines && parsed.data.sizeLines.length > 0) {
      ...
      const duplicates: { baseType: string; sizeCode: string; ingredientId: string }[] = [];
      for (const l of raw) {
    ...
    try {
      ...
      if (isUniqueViolation) {
        ...
        return {
          ...
          message:
            "Recipe save failed: duplicate or conflicting rows. Ensure all existing recipe rows for this item were removed before insert.",
          details: {
            menuItemId,
  ...
  app.patch("/categories/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    ...
    const data: { name?: string; slug?: string; sortOrder?: number } = { ...parsed.data };
  ...
  app.delete("/categories/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    if (!requireCloudAdmin(req, reply)) return;
    const { id } = req.params;
  ...
  // SubCategories
  app.post("/subcategories", async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  app.patch("/subcategories/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
  ...
  app.delete("/subcategories/:id", async (req: FastifyRequest<{ Params: { id: string }; Body?: { moveItemsToSubCategoryId?: string } }>, reply: FastifyReply) => {
    if (!requireCloudAdmin(req, reply)) return;
    const { id } = req.params;
  ...
  app.post("/items/:id/sizes", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    const parsed = z.object({
      label: z.string().min(1),
  ...
  app.patch("/items/:itemId/sizes/:sizeId", async (req: FastifyRequest<{ Params: { itemId: string; sizeId: string } }>, reply: FastifyReply) => {
  ...
  app.delete("/items/:itemId/sizes/:sizeId", async (req: FastifyRequest<{ Params: { itemId: string; sizeId: string } }>, reply: FastifyReply) => {
    const { itemId, sizeId } = req.params;
    const existing = await app.prisma.menuItemSize.findUnique({
      where: { id: sizeId },
      select: { menuItemId: true },
  ...
  app.patch("/option-groups/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    const parsed = z
      .object({
        name: z.string().min(1).optional(),
  ...
  app.delete("/option-groups/:groupId/sections/:sectionId", async (req: FastifyRequest<{ Params: { groupId: string; sectionId: string } }>, reply: FastifyReply) => {
  ...
  app.delete("/option-groups/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
  ...
  app.post("/option-groups/:groupId/options", async (req: FastifyRequest<{ Params: { groupId: string } }>, reply: FastifyReply) => {
  ...
  app.delete("/option-groups/:groupId/options/:optionId", async (req: FastifyRequest<{ Params: { groupId: string; optionId: string } }>, reply: FastifyReply) => {
  ...
  app.post("/add-on-groups", async (req: FastifyRequest, reply: FastifyReply) => {
    const parsed = z.object({
  ...
  app.patch("/add-on-groups/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    const parsed = z.object({
      name: z.string().min(1).optional(),
      isActive: z.boolean().optional(),
  ...
  app.delete("/add-on-groups/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
  ...
  app.post("/add-on-groups/:groupId/options", async (req: FastifyRequest<{ Params: { groupId: string } }>, reply: FastifyReply) => {
    const { groupId } = req.params;
    const parsed = z.object({
  ...
  app.patch("/add-on-groups/:groupId/options/:optionId", async (req: FastifyRequest<{ Params: { groupId: string; optionId: string } }>, reply: FastifyReply) => {
    const { groupId, optionId } = req.params;
    const parsed = z.object({
      name: z.string().min(1).optional(),
      priceCents: z.number().int().min(0).optional(),
  ...
  app.delete("/add-on-groups/:groupId/options/:optionId", async (req: FastifyRequest<{ Params: { groupId: string; optionId: string } }>, reply: FastifyReply) => {
    const { groupId, optionId } = req.params;
    const opt = await app.prisma.addOnOption.findFirst({ where: { id: optionId, groupId } });
    if (!opt) {
      reply.code(404);
  ...
  app.put("/add-on-groups/:groupId/options/:optionId/recipe", async (req: FastifyRequest<{ Params: { groupId: string; optionId: string } }>, reply: FastifyReply) => {
    const { groupId, optionId } = req.params;
    const opt = await app.prisma.addOnOption.findFirst({ where: { id: optionId, groupId } });
    if (!opt) {
      reply.code(404);
  ...
  app.post("/substitute-groups", async (req: FastifyRequest, reply: FastifyReply) => {
    const parsed = z.object({
      name: z.string().min(1, "Group name is required"),
      isActive: z.boolean().optional().default(true),
      sortOrder: z.number().int().optional(),
  ...
  app.patch("/substitute-groups/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    const parsed = z.object({
      name: z.string().min(1).optional(),
      isActive: z.boolean().optional(),
  ...
  app.delete("/substitute-groups/:id", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
  ...
  app.post("/substitute-groups/:groupId/options", async (req: FastifyRequest<{ Params: { groupId: string } }>, reply: FastifyReply) => {
    const { groupId } = req.params;
    const parsed = z.object({
      name: z.string().min(1, "Option name is required"),
      priceCents: z.number().int().min(0).optional(),
  ...
  app.patch("/substitute-groups/:groupId/options/:optionId", async (req: FastifyRequest<{ Params: { groupId: string; optionId: string } }>, reply: FastifyReply) => {
    const { groupId, optionId } = req.params;
    const parsed = z.object({
      name: z.string().min(1).optional(),
      priceCents: z.number().int().min(0).optional(),
  ...
  app.delete("/substitute-groups/:groupId/options/:optionId", async (req: FastifyRequest<{ Params: { groupId: string; optionId: string } }>, reply: FastifyReply) => {
    const { groupId, optionId } = req.params;
    const opt = await app.prisma.substituteOption.findFirst({ where: { id: optionId, groupId } });
    if (!opt) {
      reply.code(404);
  ...
  app.put("/substitute-groups/:groupId/options/:optionId/recipe", async (req: FastifyRequest<{ Params: { groupId: string; optionId: string } }>, reply: FastifyReply) => {
    const { groupId, optionId } = req.params;
    const opt = await app.prisma.substituteOption.findFirst({ where: { id: optionId, groupId } });
    if (!opt) {
      reply.code(404);
  ...
  app.put("/items/:id/add-on-groups", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    const parsed = z.object({ groupIds: z.array(z.string()) }).safeParse(req.body);
    if (!parsed.success) {
      reply.code(400);
  ...
  app.put("/items/:id/substitute-groups", async (req: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    const { id } = req.params;
    const parsed = z.object({
  ...
  app.get("/transactions", async (req: FastifyRequest<{ Querystring: { storeId?: string; from?: string; to?: string; limit?: string; cursor?: string } }>, reply: FastifyReply) => {
    ...
    const where: { storeId: string; createdAt?: { gte?: Date; lt?: Date } } = { storeId };
    if (fromStr || toStr) {
      where.createdAt = {};
      if (fromStr && toStr) {
        const { start, end } = localBusinessDateRangeToUtc(fromStr, toStr);
  ...
  app.post("/staff-ops/shifts", async (req: FastifyRequest, reply: FastifyReply) => {
    if (!requireCloudAdmin(req, reply)) return;
