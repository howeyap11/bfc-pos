...
function EditItemPage() {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        Promise.all([
        ...
        ]).then(([sizesFromOptions, menuSizesResult])=>{
            ...
            setDrinkSizesError("Missing drink size configuration. Run db:seed in cloud-api and ensure Menu Settings → Sizes is configured.");
        });
    }, []);
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        Promise.all([
            __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].getItem(id, includeDeleted),
            __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].getRecipe(id),
            ...
            __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].getOptionGroups()
        ]).then(([i, recipe, ings, cats, groups])=>{
            setItem(i);
            setName(i.name);
            setPricePesos((i.priceCents / 100).toFixed(2));
            setIsActive(i.isActive);
            setCategoryId(i.categoryId ?? i.subCategory?.categoryId ?? "");
            setSubCategoryId(i.subCategoryId ?? "");
            const configs = i.drinkSizeConfigs ?? [];
            const defaults = i.drinkModeDefaults ?? [];
            const initialHasSizes = i.hasSizes ?? (configs.length > 0 || defaults.length > 0);
            if (configs.length > 0 || defaults.length > 0) {
                const next = {
                    ...DEFAULT_DRINK_SIZES_BY_MODE
                };
                for (const mode of DRINK_MODES){
                    next[mode].enabledOptionIds = configs.filter((c)=>c.mode === mode && c.isEnabled).map((c)=>c.optionId);
                    const def = defaults.find((d)=>d.mode === mode);
                    next[mode].defaultOptionId = def ? def.defaultOptionId : null;
                }
                setDrinkSizesByMode(next);
                setHasSizes(initialHasSizes && DRINK_MODES.some((mode)=>next[mode].enabledOptionIds.length > 0));
            } else {
                setDrinkSizesByMode(DEFAULT_DRINK_SIZES_BY_MODE);
                setHasSizes(initialHasSizes);
            }
            // Initialize sizePricesByMode from item.sizePrices (if any)
            const sizePrices = i.sizePrices ?? [];
            if (sizePrices.length > 0) {
                const byMode = {};
                for (const mode of DRINK_MODES){
                    const entries = sizePrices.filter((p)=>p.baseType === mode);
                    if (entries.length > 0) {
                        const map = {};
                        for (const p of entries){
                            map[p.sizeOptionId] = p.priceCents;
                        }
                        byMode[mode] = map;
                    }
                }
            ...
            setSelectedGroupIds((i.optionGroupLinks ?? []).map((l)=>l.groupId));
            setRecipeLines((recipe.lines ?? []).map((r)=>({
                    ingredientId: r.ingredientId,
                    qtyPerItem: parseQty(r.qtyPerItem),
                    unitCode: r.unitCode
                })));
            setSizeRecipeLines((recipe.sizeLines ?? []).map((r)=>({
                    ingredientId: r.ingredientId,
                    baseType: r.baseType,
                    sizeCode: r.sizeCode,
                    qtyPerItem: parseQty(r.qtyPerItem),
            ...
            setCategories((0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["normalizeCategoriesResponse"])(cats).categories);
            setOptionGroups(Array.isArray(groups) ? groups : []);
            if (ings.length) setNewUnitCode(ings[0].unitCode);
    ...
    async function saveItem(e) {
        ...
        if (hasSizes) {
            const modesWithEnabledNoDefault = DRINK_MODES.filter((mode)=>drinkSizesByMode[mode].enabledOptionIds.length > 0 && !drinkSizesByMode[mode].defaultOptionId);
            if (modesWithEnabledNoDefault.length > 0) {
                setError(`Select a default size for: ${modesWithEnabledNoDefault.map((m)=>MODE_LABELS[m]).join(", ")}.`);
                return;
            }
            // When item has sizes, require a price for every enabled size
            const missingPrices = [];
            for (const mode of DRINK_MODES){
                const { enabledOptionIds } = drinkSizesByMode[mode];
                if (enabledOptionIds.length === 0) continue;
                const modePrices = sizePricesByMode?.[mode] ?? {};
                for (const optId of enabledOptionIds){
                    const raw = modePrices[optId];
                    if (raw === undefined || Number.isNaN(raw) || raw < 0) {
                        missingPrices.push(`${MODE_LABELS[mode]} - option ${optId}`);
                    }
                }
            }
            if (missingPrices.length > 0) {
                setError("Enter a non-negative price for every enabled size.");
                return;
            }
        }
        // Derive base price cents. For size-based items, use the first default size price if available.
        let priceCents = Math.round(peso * 100);
        if (hasSizes) {
            let derived = null;
            for (const mode of DRINK_MODES){
                const { defaultOptionId } = drinkSizesByMode[mode];
                if (defaultOptionId) {
                    const cents = sizePricesByMode?.[mode]?.[defaultOptionId];
            ...
            if (derived === null) {
                // Fallback: minimum of all configured size prices
                let min = null;
                for (const mode of DRINK_MODES){
                    const map = sizePricesByMode?.[mode];
                    if (!map) continue;
        ...
        setSaving(true);
        try {
            await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].updateItem(id, {
                name,
                priceCents,
                isActive,
            ...
            // Always persist drink sizes: when hasSizes is false we send all empty to clear config
            const payload = hasSizes ? drinkSizesByMode : DEFAULT_DRINK_SIZES_BY_MODE;
            const sizePricesPayload = hasSizes ? sizePricesByMode : undefined;
            await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].putItemDrinkSizes(id, {
                drinkSizesByMode: payload,
                hasSizes,
                sizePricesByMode: sizePricesPayload
            });
            if (imageFile) {
                await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].uploadItemImage(id, imageFile);
                setImageFile(null);
            }
            await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].putItemOptionGroups(id, selectedGroupIds);
            setItem(null);
            const [i] = await Promise.all([
                __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].getItem(id)
            ]);
            setItem(i);
        } catch (err) {
            const body = err?.body;
            setError(body?.message ?? (err instanceof Error ? err.message : "Failed to update. Check category/subcategory."));
        } finally{
            setSaving(false);
        }
    }
    async function saveRecipe() {
        ...
        setSaving(true);
        try {
            const baseLinesPayload = recipeLines.map((r)=>({
                    ingredientId: r.ingredientId,
                    qtyPerItem: r.qtyPerItem,
                    unitCode: r.unitCode
                }));
            const sizeLinesPayload = hasSizes && sizeRecipeLines.length > 0 ? sizeRecipeLines.filter((r)=>r.qtyPerItem > 0).map((r)=>({
                    ingredientId: r.ingredientId,
                    baseType: r.baseType,
                    sizeCode: r.sizeCode,
                    qtyPerItem: r.qtyPerItem,
            ...
            await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["api"].putRecipe(id, baseLinesPayload, sizeLinesPayload);
        } catch (err) {
    ...
    function removeRecipeLine(index) {
        ...
        setSizeRecipeLines((prev)=>{
            const ingredientId = recipeLines[index]?.ingredientId;
    ...
    const ingredientName = (ingredientId)=>ingredients.find((i)=>i.id === ingredientId)?.name ?? ingredientId;
    const safeCategories = Array.isArray(categories) ? categories : [];
    const subCategories = categoryId ? (0, __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$cloud$2d$ui$2f$src$2f$lib$2f$api$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["normalizeSubCategories"])(safeCategories.find((c)=>c.id === categoryId)?.subCategories) : [];
    function setModeEnabled(mode, optionId, enabled) {
        setDrinkSizesByMode((prev)=>{
            const next = {
                ...prev
            };
            const enabledIds = enabled ? [
                ...prev[mode].enabledOptionIds,
                optionId
            ] : prev[mode].enabledOptionIds.filter((id)=>id !== optionId);
            let defaultId = prev[mode].defaultOptionId;
            if (!enabled && defaultId === optionId) defaultId = null;
            next[mode] = {
    ...
    function setModeDefault(mode, defaultOptionId) {
        setDrinkSizesByMode((prev)=>({
                ...prev,
                [mode]: {
                    ...prev[mode],
    ...
    async function handleDeleteItem() {
        ...
        try {
            ...
            setError(msg);
            console.error("[handleDeleteItem] failed:", {
                itemId: id,
                error: err
            });
            setSaving(false);
                        ...
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            ...
                            children: [
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("button", {
                                    ...
                                    children: saving ? "Restoring…" : "Restore item"
                                }, void 0, false, {
                                    fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                    lineNumber: 421,
                                    columnNumber: 13
    ...
    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
        ...
        children: [
            ...
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("form", {
                onSubmit: saveItem,
                className: "mb-8 max-w-md space-y-4 rounded border bg-white p-4",
                children: [
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        children: [
                            ...
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                ...
                                children: [
                                    imagePreview ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("img", {
                                        ...
                                        lineNumber: 458,
                                        columnNumber: 15
                                    }, this) : item.imageUrl ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("img", {
                    ...
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        ...
                        children: [
                                ...
                                children: [
                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("input", {
                                        ...
                                        onChange: (e)=>{
                                            ...
                                            setHasSizes(checked);
                                            if (!checked) setDrinkSizesByMode(DEFAULT_DRINK_SIZES_BY_MODE);
                                        },
                            ...
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                ...
                                children: "When checked, this item can have size options (e.g. 12oz, 16oz) per mode. When unchecked, the item behaves as a normal non-size item."
                            }, void 0, false, {
                                fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                lineNumber: 558,
                                columnNumber: 11
                            ...
                            hasSizes && drinkSizes && drinkSizes.options.length > 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                ...
                                children: [
                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                        ...
                                        children: "Enable sizes and set prices per mode. Price is required when a size is enabled."
                                    }, void 0, false, {
                                        fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                        lineNumber: 574,
                                        columnNumber: 15
                                    }, this),
                                    DRINK_MODES.map((mode)=>{
                                        const { enabledOptionIds, defaultOptionId } = drinkSizesByMode[mode];
                                        const enabledForDefault = drinkSizes.options.filter((o)=>enabledOptionIds.includes(o.id));
                                        const hasEnabledNoDefault = enabledOptionIds.length > 0 && !defaultOptionId;
                                        const modePrices = sizePricesByMode?.[mode] ?? {};
                                        // Derive which size labels are globally available for this mode from Menu Settings
                                        const idsForMode = sizeAvailability[mode] ?? [];
                                        const sizeById = new Map(menuSizes.map((s)=>[
                                                s.id,
                                                s.label
                                            ]));
                                        const allowedLabels = new Set(idsForMode.map((id)=>sizeById.get(id)).filter((label)=>!!label));
                                        const modeOptions = allowedLabels.size > 0 ? drinkSizes.options.filter((o)=>allowedLabels.has(o.label)) : drinkSizes.options;
                                        return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                        ...
                                                        return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                            ...
                                                            children: [
                                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("label", {
                                                                    ...
                                                                    children: [
                                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("input", {
                                                                            type: "checkbox",
                                                                            checked: enabled,
                                                                            onChange: (e)=>setModeEnabled(mode, o.id, e.target.checked),
                                                                            className: "rounded border-gray-300"
                                                                        }, void 0, false, {
                                                                            fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                                                            lineNumber: 606,
                                                                ...
                                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("input", {
                                                                    ...
                                                                    step: "0.01",
                                                                    disabled: !enabled,
                                                                    value: typeof modePrices[o.id] === "number" ? (modePrices[o.id] / 100).toFixed(2) : "",
                                                                    onChange: (e)=>{
                                                                        const next = {
                                                                            ...sizePricesByMode ?? {}
                                                                        };
                                                                        const raw = e.target.value;
                                                                        const pesos = raw === "" ? null : parseFloat(raw);
                                                                        const currentMode = next[mode] ?? {};
                                                                        if (pesos === null || Number.isNaN(pesos) || pesos < 0) {
                                                                            delete currentMode[o.id];
                                                                        } else {
                                                                            currentMode[o.id] = Math.round(pesos * 100);
                                                                        }
                                                                        next[mode] = currentMode;
                                                                        setSizePricesByMode(next);
                                                                    },
                                                ...
                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                                    ...
                                                    children: [
                                                        ...
                                                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("select", {
                                                            ...
                                                            onChange: (e)=>setModeDefault(mode, e.target.value || null),
                                                            disabled: enabledOptionIds.length === 0,
                                                            className: "w-full rounded border border-gray-300 px-2 py-1.5 text-sm disabled:bg-gray-100 disabled:text-gray-500",
                                                            children: [
                                                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("option", {
                                                                    value: "",
                                                                    children: enabledOptionIds.length === 0 ? "No sizes enabled" : "Select default"
                                                                }, void 0, false, {
                                                                    fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                                                    lineNumber: 652,
                                                                    columnNumber: 25
                                                                }, this),
                                                                enabledForDefault.map((o)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("option", {
                                                                        value: o.id,
                                                                        children: o.label
                                                                    }, o.id, false, {
                                                                        fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                                        ...
                                                        hasEnabledNoDefault && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                                            className: "mt-1 text-xs text-amber-600",
                                                            children: "Select a default size for this mode."
                                                        }, void 0, false, {
                                                            fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                                            lineNumber: 660,
                                                            columnNumber: 25
                    ...
                    hasSizes && drinkSizes && drinkSizes.options.length > 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        ...
                        children: [
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("h3", {
                                ...
                                children: "Per-size recipe (qty per drink)"
                            }, void 0, false, {
                                fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                lineNumber: 781,
                                columnNumber: 13
                            ...
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                ...
                                children: "When sizes are enabled, you can specify how much of each ingredient is used for every size. Leave a cell blank or zero to indicate no additional consumption for that size."
                            }, void 0, false, {
                                fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                lineNumber: 782,
                                columnNumber: 13
                            ...
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                ...
                                children: recipeLines.length === 0 ? /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                                    className: "text-xs text-gray-500",
                                    children: "Add ingredients above to define per-size consumption."
                                }, void 0, false, {
                                    fileName: "[project]/apps/cloud-ui/src/app/items/[id]/page.tsx",
                                    lineNumber: 788,
                                    columnNumber: 17
                                }, this) : DRINK_MODES.map((mode)=>{
                                    const enabledIds = drinkSizesByMode[mode].enabledOptionIds;
                                    if (enabledIds.length === 0) return null;
                                    const columns = drinkSizes.options.filter((o)=>enabledIds.includes(o.id));
                                        ...
                                        children: [
                                            ...
                                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("table", {
                                                ...
                                                children: [
                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("thead", {
                                                        ...
                                                        children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("tr", {
                                                            children: [
                                                                ...
                                                                columns.map((col)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("th", {
                                                    ...
                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("tbody", {
                                                        className: "divide-y divide-gray-200",
                                                        children: recipeLines.map((line)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("tr", {
                                                                children: [
                                                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("td", {
                                                                    ...
                                                                    columns.map((col)=>{
                                                                        const existing = sizeRecipeLines.find((r)=>r.ingredientId === line.ingredientId && r.baseType === mode && r.sizeCode === col.label);
                                                                        const value = existing ? existing.qtyPerItem.toString() : "";
                                                                        return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("td", {
                                                                            ...
                                                                            children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f$next$40$16$2e$1$2e$6_$40$babel$2b$core$40$7$2e$2_9d8d1bf7a8807769963b5151bd760c41$2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("input", {
                                                                                ...
                                                                                onChange: (e)=>{
                                                                                    ...
                                                                                    setSizeRecipeLines((prev)=>{
                                                                                        ...
                                                                                        const idx = next.findIndex((r)=>r.ingredientId === line.ingredientId && r.baseType === mode && r.sizeCode === col.label);
...
//# sourceMappingURL=apps_cloud-ui_src_app_items_%5Bid%5D_page_tsx_fbef4ccd._.js.map