...
const TABS = ["Main", "Transactions", "Hourly", "Daily", "Monthly"] as const;
...
function ReportsContent() {
  ...
  const activeTab = TABS.includes(tabParam as TabId) ? (tabParam as TabId) : "Main";
  ...
  useEffect(() => {
    ...
    setFrom((f) => f || weekAgo.toISOString().slice(0, 10));
    setTo((t) => t || today.toISOString().slice(0, 10));
  }, []);
        ...
        <div style={{ background: COLORS.bgPanel, borderRadius: 8, border: `1px solid ${COLORS.borderLight}`, overflow: "hidden" }}>
          {activeTab === "Main" && (
            <div className="p-8 text-center text-white/70">
              <p>Select a tab above to view transactions, hourly, daily, or monthly reports.</p>
            </div>
          )}
