...
import { api, type AddOnGroup, type AddOnOption, type Ingredient, type IngredientCategory } from "@/lib/api";
...
export default function AddOnsPage() {
  ...
  const [recipeModal, setRecipeModal] = useState<{ groupId: string; option: AddOnOption } | null>(null);
  ...
  async function handleCreateGroup(e: React.FormEvent) {
    ...
    setError("");
    try {
      await api.createAddOnGroup({ name: newGroupName.trim() });
      setNewGroupName("");
  ...
  async function handleDeleteGroup(g: AddOnGroup) {
    if (!confirm(`Delete add-on group "${g.name}"?`)) return;
    setError("");
    try {
      await api.deleteAddOnGroup(g.id);
  ...
  async function handleCreateOption(groupId: string, e: React.FormEvent) {
    ...
    try {
      ...
      await api.createAddOnOption(groupId, { name: newOptionName.trim(), priceCents });
  ...
  async function handleUpdateOption(e: React.FormEvent) {
    ...
    try {
      ...
      await api.patchAddOnOption(editOption.groupId, editOption.option.id, { name: editOptionName.trim(), priceCents, isActive: editOptionActive });
      setEditOption(null);
  ...
  async function handleDeleteOption(groupId: string, opt: AddOnOption) {
    if (!confirm(`Delete option "${opt.name}"?`)) return;
    setError("");
    try {
      await api.deleteAddOnOption(groupId, opt.id);
  ...
  function openRecipe(groupId: string, option: AddOnOption) {
  ...
  async function saveRecipe() {
    ...
    setError("");
    try {
      await api.putAddOnOptionRecipe(recipeModal.groupId, recipeModal.option.id, recipeLines);
      setRecipeModal(null);
  ...
  return (
    <div className="mx-auto max-w-5xl py-6">
      <h1 className="mb-4 text-2xl font-semibold">Add-on Groups</h1>
      <p className="mb-6 text-sm text-gray-600">
        Group add-ons (e.g. Extra Toppings) with options inside. Assign groups to items in Create/Edit Item.
      </p>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {groups.length === 0 && <p className="py-8 text-center text-gray-500">No add-on groups yet. Add one above.</p>}
        </div>
      )}
