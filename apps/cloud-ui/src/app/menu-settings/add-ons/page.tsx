...
import { api, type AddOn, type Ingredient, type IngredientCategory } from "@/lib/api";
...
export default function AddOnsPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      await api.createAddOn({ name: newName.trim(), priceCents });
      setNewName("");
      setNewPrice("");
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      ...
      await api.patchAddOn(editId, { name: editName.trim(), priceCents, isActive: editActive });
      setEditId(null);
  ...
  function openRecipe(addOn: AddOn) {
    setRecipeModalId(addOn.id);
    setRecipeLines(
      (addOn.recipeLines ?? []).map((r) => ({
  ...
  async function saveRecipe() {
    ...
    setError("");
    try {
      await api.putAddOnRecipe(recipeModalId, recipeLines);
  ...
  return (
    <div className="mx-auto max-w-5xl py-6">
      <h1 className="mb-4 text-2xl font-semibold">Add-ons</h1>
      <p className="mb-6 text-sm text-gray-600">
        Optional extras with prices and optional recipe consumption (e.g. extra milk, syrup, sauce). Assign to items in Create/Edit Item.
      </p>
      ...
      <div className="mb-8 rounded-lg border border-gray-200 bg-white p-4">
        <form onSubmit={handleCreate} className="flex flex-wrap items-end gap-3">
          <div className="min-w-[200px] flex-1">
            <label className="mb-1 block text-sm font-medium text-gray-700">Add Add-on</label>
            <input
              type="text"
              value={newName}
              onChange={(e) => setNewName(e.target.value)}
              placeholder="e.g. Extra Milk, Vanilla Syrup"
      ...
      {loading ? (
        ...
        <div className="space-y-4">
              ...
              {editId === addOn.id ? (
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <span className="ml-2 text-gray-500">₱{((addOn.priceCents ?? 0) / 100).toFixed(2)}</span>
                    {!addOn.isActive && <span className="ml-2 text-amber-600">(inactive)</span>}
                  </div>
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => openRecipe(addOn)}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setEditPrice(((addOn.priceCents ?? 0) / 100).toFixed(2));
                        setEditActive(addOn.isActive ?? true);
                      }}
                      className="rounded border border-gray-300 px-3 py-1.5 text-sm hover:bg-gray-100"
                    >
                      Edit
                    </button>
                    <button type="button" onClick={() => handleDelete(addOn)} className="text-red-600 hover:underline">
                      Delete
                    </button>
                  </div>
                </div>
          ...
          {addOns.length === 0 && <p className="py-8 text-center text-gray-500">No add-ons yet. Add one above.</p>}
        </div>
      )}
