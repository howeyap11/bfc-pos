...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    logProxyRequest("POST /api/pos/transactions/:id/print-receipt", req, { transactionId: id });

    const upstream = await fetch(`${getBackendUrl()}/pos/transactions/${id}/print-receipt`, {
      method: "POST",
